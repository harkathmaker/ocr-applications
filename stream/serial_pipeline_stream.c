...
ocrGuid_t pipeEdt(u32 paramc, u64 * paramv, u32 depc, ocrEdtDep_t depv[]) {
...
ocrGuid_t iterEdt(u32 paramc, u64 * paramv, u32 depc, ocrEdtDep_t depv[]) {
	...
	ocrEdtTemplateCreate(&pipeTemplateGuid, &pipeEdt, paramc, 1);

	// EDT for pipeline
	ocrGuid_t pipeGuid, pipeDone;
	ocrEdtCreate(&pipeGuid, pipeTemplateGuid, EDT_PARAM_DEF, paramv, EDT_PARAM_DEF, NULL_GUID,
		EDT_PROP_FINISH, NULL_GUID, &pipeDone);
	...
	if (paramv[0] < NTIMES) {
		...
		ocrEdtTemplateCreate(&iterTemplateGuid, &iterEdt, paramc, 2);
		...
		ocrAddDependence(pipeDone, iterGuid, 1, DB_MODE_RO);
	} 
	ocrAddDependence(dataGuid, pipeGuid, 0, DB_MODE_EW);
	return NULL_GUID;
}
...
ocrGuid_t mainEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
	...
	ocrGuid_t iterTemplateGuid;
	ocrEdtTemplateCreate(&iterTemplateGuid, &iterEdt, 1, 1);
